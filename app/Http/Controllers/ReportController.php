...
class ReportController extends Controller
{
    ...
    public function show(int $id)
    {
        try {
            //code...
            $masterSql = "select s.sale_no [no], s.created_at [date], s.remarks, c.name , c.address , c.city, c.pincode, c.phone, c.gst, s.ref_no, s.ref_date
            from sales s
            inner join contacts c on s.contact_id = c.id
            where s.id =" . $id;
            $sale = DB::select($masterSql);

            $detailsSql = "select si.s_no,cl.name [cloth], co.name [color], si.weight, si.actual_weight, si.rate, si.amount
            from sale_items si
